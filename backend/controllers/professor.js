...
const getAssignedClasses = async (req, res) => {
    ...
    const professor = await pool.one(
        `SELECT id FROM professors WHERE user_id = $1`,
        [userId]
    );

    const classes = await pool.any(
        `
    SELECT
      c.id AS course_id,
      c.course_code,
...
const getStudents = (req,res)=>{
    try{
        ...
        const studentRes = pool.query('select student_id, first_name, last_name from students where group_number = $1',[groupNumber]);
        if(!studentRes){
...
module.exports = { loginProfessor, professorDetails, registerProfessor, getAssignedClasses,getStudents };